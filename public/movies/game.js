...
function attachAutocomplete(input, getNames, onPick, opts = {}){
  ...
  // immediately, but touch browsers then synthesize a `click` at those
  // coordinates — and since the menu is already gone, that click lands on
  // whatever is now underneath (a button or a modal) and fires it. Arm a
  // one-shot capture-phase listener that eats exactly that ghost click; a short
  // timeout disarms it so a genuine later tap is never swallowed.
