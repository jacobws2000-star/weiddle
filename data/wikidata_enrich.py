...
def roster_targets():
    ...
    for aid in ids:
        ...
        if need_nat or need_dob:
            targets.append((name, need_nat, need_dob))
...
def main(limit=None):
    ...
    print(f"[targets] {len(targets)} fighters missing nationality and/or DOB", file=sys.stderr)

    # Pass 1: pull each fighter's raw claims.
    raw = {}   # name -> {p27, p1532, p19, dob, need_nat, need_dob}
    for i, (name, need_nat, need_dob) in enumerate(targets):
        if i % 25 == 0:
        ...
        raw[name] = {
            ...
            "need_nat": need_nat, "need_dob": need_dob,
        }
    _save()
    ...
    print(f"\n[done] nationality: {len(NAT)}   dob: {len(DOB)}", file=sys.stderr)
